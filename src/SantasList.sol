...
contract SantasList is ERC721, TokenUri {
    ...
    function checkList(address person, Status status) public {
        s_theListCheckedOnce[person] = status; // this is  a bug...it should return the status of the person and not equate it, anyone can call this contract..
     ...
     * @param status The status of the person
     */
    function checkTwice(address person, Status status) public onlySanta {
    ...
    function collectPresent() public{
        ...
        if (balanceOf(msg.sender) > 0) {
            // indicate the nft balance of the sender
        ...
        revert SantasList__NotNice(); // this is fine
    }
     ...
     */
    function buyPresent(address presentReceiver) public {
        // modifier is needed here for someone naughty and this person must have at least 2e18 of snata token...
        i_santaToken.burn(presentReceiver); // you can only burn from anextremy nice child...else it would break
        _mintAndIncrement(); // here the naughty child gets to mint an nft...
    }
    ...
    function getOwnerOf(uint256 amount) public view returns (address) {
        return _ownerOf(amount);
    }

    function getTokenCounter() public view returns (uint256 amount) {
